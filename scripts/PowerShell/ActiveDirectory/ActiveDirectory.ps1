...
function Check-DomainExistance {
  ...
  Param(
   ...
   $DCs = Get-ADDomainController -Filter * -Server $DomainName | Select-Object Hostname,isGlobalCatalog,IsReadOnly

   # Create empty Array object
   $DCArray = New-Object System.Collections.ArrayList
   
   # Create an Array of DC for the provided doamain
   ForEach($DC in $DCs)
   { 
   #If this DC is not read only we can add it to the list
   if(-Not $DC.ReadOnly) {
       $DCArray.Add($DC.Hostname) | Out-Null
   }
  }
  return $DCArray
}
...
function Configure-ActiveDirectory {
  ...
  Import-Module ADDSDeployment
   ...
   $credential = New-Object System.Management.Automation.PSCredential("$DomainName\$Username",$Password)
...
if (Check-DomainExistance){
  ...
  Configure-ActiveDirectory -DomainName "nucalm.com" -Password $secpasswd -DomainMode $DomainMode -ForestMode $DomainMode -DomainType DC -ChildDomainName ""
}
